...
if(!empty($_GET['email']) && !empty($_GET['gallery']) && empty($_GET['password']))
{
	$gid = base64_decode($_GET['gallery']);	
	$conditions = array('gallery'=>$gid, 'status'=>'1');
	$photo = $common->getrecord('pr_photos','*',$conditions);
	$linkid = base64_encode($photo->id);	
	$mainlink = APP_URL."view-photo.php?view=$linkid";
	$common->redirect($mainlink);
}
...
if(!empty($_GET['password']))
{
	...
	if(!empty($check))	
	{
		...
		$conditions = array('gallery'=>$id, 'status'=>'1');
		$photo = $common->getrecord('pr_photos','*',$conditions);
		$linkid = base64_encode($photo->id);	
		$mainlink = APP_URL."view-photo.php?view=$linkid";
		$common->redirect($mainlink);
						...
						<div class="photo_width_photos1">
							<div class="photo_width_photos">
								<div class="bottom-grids" style="margin-top:0px;">
									...
									{
										?>
										<div style="font-weight:bold; padding-left:15px; margin:7px;">Public Free File $0.00 USD</div>
										<?php
									}
									else
									{
										?>
										<div style="font-weight:bold; padding-left:15px; margin:7px;">Public Premium File $<?php echo $photo->webfileprice; ?> USD</div>
										<?php
									}
									?>
								</div>	
						...
						<div class="photo_width_photos1">
							<div class="photo_width_photos">
								<div class="bottom-grids" style="margin-top:0px;">
									...
									{
										?>
										<div style="font-weight:bold; padding-left:15px; margin:7px;">Private Free File $0.00 USD</div>
										<?php
									}
									else
									{
										?>
										<div style="font-weight:bold; padding-left:15px; margin:7px;">Private Premium File $<?php echo $photo1->webfileprice; ?> USD</div>
										<?php
									}
									?>
								</div>	
